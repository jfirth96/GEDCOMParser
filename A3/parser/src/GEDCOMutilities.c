...
int addPersonWrap( char* individual, char* file ) {
    if (file == NULL || individual == NULL) {
        return -1;
    }
    // Open file
    GEDCOMobject *obj = NULL;
    char *s = calloc( sizeof( char ), strlen( file ) + strlen( "uploads/" ) + 2 );
    strcpy( s, "uploads/" );
    strcat( s, file );
    GEDCOMerror error = createGEDCOM( s, &obj );
    ...
    // Create individual
    Individual *indi = JSONtoInd( individual );
    if (indi == NULL) {
        return -2; // unable to create individual
    }

    // addIndiv() if ! exists already
    Individual *refPtr = findPerson( obj, &surnamePredicate, indi );
    if (refPtr == NULL) {
        addIndividual( obj, indi );
    } else {
        deleteGEDCOM( obj );
        deleteIndividual( indi );
        return -3; // person already exists
    }

    // validateGEDCOM()
    ErrorCode valid = validateGEDCOM( obj );
    if (valid != OK) {
        deleteGEDCOM( obj );
        deleteIndividual( indi );
        return -4; // object is now invalid
    }

    // writeGEDCOM()
    GEDCOMerror writeError = writeGEDCOM( s, obj );
    free( s );
    if (writeError.type != OK) {
        deleteGEDCOM( obj );
        deleteIndividual( indi );
        return -5; // unable to write file
    }
...
char* GEDCOMtoJSON( char *filename ) {
    ...
    GEDCOMobject *obj = NULL;
    char *tmp = calloc( sizeof( char ), strlen( filename ) + strlen( "uploads/" ) + 2 );
    strcpy( tmp, "uploads/" );
    strcat( tmp, filename );
    //printf( "C func: %s\n", tmp );
    GEDCOMerror err = createGEDCOM( tmp, &obj );
    free( tmp );

    char *str = calloc( sizeof( char ), 3 );
    if (err.type != OK || obj == NULL) {
        strcpy( str, "{}" );
        return str;
    }
    str[0] = '{';
    ...
    //file name
    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( "\"file\":\"" ) + 1) );
    strcat( str, "\"file\":\"" );
    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( filename ) + 1) );
    strcat( str, filename );
    str = realloc( str, sizeof( char ) * (strlen( str ) + 3) );
    strcat( str, "\"," );

    // Header
    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( "\"source\":\"" ) + 1) );
    strcat( str, "\"source\":\"" );
    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( obj->header->source ) + 1) );
    strcat( str, obj->header->source );
    str = realloc( str, sizeof( char ) * (strlen( str ) + 3) );
    strcat( str, "\"," );

    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( "\"vers\":\"" ) + 1) );
    strcat( str, "\"vers\":\"" );
    str = realloc( str, sizeof( char ) * (strlen( str ) + 4) );
    char s[5];
    sprintf( s, "%.2f", obj->header->gedcVersion );
    strcat( str, s );
    str = realloc( str, sizeof( char ) * (strlen( str ) + 3) );
    strcat( str, "\"," );

    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( "\"charSet\":\"" ) + 1) );
    strcat( str, "\"charSet\":\"" );
    char *p = getStringFromCharSet( obj->header->encoding );
    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( p ) + 1) );
    strcat( str, p );
    free( p );
    str = realloc( str, sizeof( char ) * (strlen( str ) + 3) );
    strcat( str, "\"," );
    ...
    // Submitter
    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( "\"subName\":\"" ) + 1) );
    strcat( str, "\"subName\":\"" );
    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( obj->submitter->submitterName ) + 1) );
    strcat( str, obj->submitter->submitterName );
    str = realloc( str, sizeof( char ) * (strlen( str ) + 3) );
    strcat( str, "\"," );

    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( "\"subAddr\":\"" ) + 1) );
    strcat( str, "\"subAddr\":\"" );
    if (strlen( obj->submitter->address ) > 0) {
        str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( obj->submitter->address ) + 1) );
    ...
    sprintf( s, "%d", famCount );
    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( "\"famCount\":\"" ) + 1) );
    strcat( str, "\"famCount\":\"" );
    ...
    // # indivs
    int indivCount = getLength( obj->individuals );
    sprintf( s, "%d", indivCount );
    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( "\"indivCount\":\"" ) + 1) );
    strcat( str, "\"indivCount\":\"" );
    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( s ) + 1) );
    strcat( str, s );
    str = realloc( str, sizeof( char ) * (strlen( str ) + 3) );
    strcat( str, "\"," );

    str[strlen( str ) - 1] = '}'; // replace trailing comma
    deleteGEDCOM( obj );
    return str;
}
...
char* getDescendantsWrap( char *filename, char *individual, int maxGen ) {
    ...
    // get descendants
    List descendants = getDescendantListN( obj, refInd, maxGen );
    ...
    // glist to json
    char *descendListJSON = gListToJSON( descendants );
    clearList( &descendants );
	deleteGEDCOM( obj );
	deleteIndividual( target );
    if (descendListJSON == NULL) {
		return str;
	} else {
		free( str );
		return descendListJSON;
	}
}
...
char* indivListToJSON( char* filename ) {
    char *str = calloc( sizeof( char ), 3 );
    str[0] = '{';
    str[1] = '}';
    if (filename == NULL) {
    ...
    // Turn iList into JSON
    char *indivList = iListToJSON( obj->individuals );
    free( str );
    return indivList;
}
...
int JSONtoGEDCOMWrap( char *json, char *file ) {
    ...
    if (valid == OK) {
        ...
        strcpy( s, "uploads/" );
        strcat( s, file );
        GEDCOMerror write = writeGEDCOM( s, obj );
        free( s );
        deleteGEDCOM( obj );
        if (write.type == OK) {
            return 1;
...
GEDCOMline* parseGEDCOMline( char* line ) {
    ...
    GEDCOMline* toReturn = calloc( sizeof( GEDCOMline ), 1 );
