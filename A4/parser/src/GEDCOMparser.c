...
GEDCOMerror createGEDCOM( char* fileName, GEDCOMobject** obj ) {
    ...
    if (fileName == NULL || strlen( fileName ) == 0) {
        ...
        char* temp;
        char* copy = calloc( sizeof( char ), (strlen( fileName ) + 1) );
        strcpy( copy, fileName );
        temp = strtok( copy, "." ); // should contain the file name portion
        if (temp == NULL) {
            free( copy );
            fileError = true;
            goto FILE_ERROR;
            ...
            if (getLength( indiv->families ) > 0) {
                ...
                while ((famElem = nextElement( &famIter )) != NULL) {
                    ...
                    if (index2 <= getLength( fam->children ) - 1) {
                        // NULL pointer encountered before end of children list
                        return INV_RECORD;
                    }
                }
                if (index1 <= getLength( indiv->families ) - 1) {
                    // NULL pointer encountered before end of family list
                    return INV_RECORD;
                }
            }
        }
        if (index <= getLength( obj->individuals ) - 1) {
            // NULL pointer encountered before end of individual list
            return INV_RECORD;
        }
    }
...
char* indToJSON(const Individual* ind) {
    ...
    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( "\"sex\":\"" ) + 1) );
    strcat( str, "\"sex\":\"" );
    if (getLength( ind->otherFields ) > 0) {
        ...
        while ((data = nextElement( &iter )) != NULL) {
            ...
            if (strcmp( f->tag, "SEX" ) == 0) {
                if (strcmp( f->value, "M" ) == 0) {
                    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( "Male" ) + 1) );
                    strcat( str, "Male" );
                } else {
                    str = realloc( str, sizeof( char ) * (strlen( str ) + strlen( "Female" ) + 1) );
                    strcat( str, "Female" );
                }
                break;
            } 
...
Individual* JSONtoInd(const char* str) {
    ...
    // This inserts a character between adjacent delimiters to make my life easier when parsing later
    ...
    indiv->families = initializeList( &printFamily, &dummyDelete, &compareFamilies );
